...
--*/
{
	DPF_ENTER(("[CMiniportWaveRTStream::Init](Pin = %d)", Pin_));
    PAGED_CODE();
...
--*/
{
	DPF_ENTER(("[CMiniportWaveRTStream::NonDelegatingQueryInterface]"));
    PAGED_CODE();
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::AllocateBufferWithNotification(IMiniportWaveRTStream interface)]"));
	DPF_ENTER(("[AllocateBufferWithNotification(RequsetSize = %d)(NotificationCount = %d)]",RequestedSize_,NotificationCount_));
    PAGED_CODE();
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::FreeBufferWithNotification(IMiniportWaveRTStream interface)]"));
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::RegisterNotificationEvent(IMiniportWaveRTStream interface)]"));
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::UnregisterNotificationEvent(IMiniportWaveRTStream interface)]"));
    UNREFERENCED_PARAMETER(NotificationEvent_);
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetClockRegister(IMiniportWaveRTStream interface)]"));
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetPositionRegister(IMiniportWaveRTStream interface)]"));
    UNREFERENCED_PARAMETER(Register_);
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetHWLatency(IMiniportWaveRTStream interface)]"));
    PAGED_CODE();
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::FreeAudioBuffer(IMiniportWaveRTStream interface)]"));
    UNREFERENCED_PARAMETER(Size_);
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::AllocateAudioBuffer(IMiniportWaveRTStream interface)]"));
    PAGED_CODE();
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetPosition(IMiniportWaveRTStream Interface)]"));
    NTSTATUS ntStatus;
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetReadPacket(IMiniportWaveRTOutputStream interface)]"));
    NTSTATUS ntStatus;
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::SetWritePacket(IMiniportWaveRTOutputStream interface)]"));
	DPF_ENTER(("[CMiniportWaveRTStream::SetWritePacket(IMiniportWaveRTOutputStream interface)(PacketNumber = %d)(Flags = %d)(EosPacketLength = %d)]", PacketNumber, Flags, EosPacketLength));
    NTSTATUS ntStatus;
    ...
    else
    {
		...
		DPF_ENTER(("[CMiniportWaveRTStream::SetWritePacket::SetCurrentWritePositionInternal]"));
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetOutputStreamPresentationPosition(IMiniportWaveRTOutputStream interface)(call LinearBufferPosition)]"));
...
GetPacketCount returns the (1-based) count of packets completely transferred from the 
WaveRT buffer into hardware.

Remarks:
	from the packet count, the OS can derive the stream position of he packets it writes into
	the WaveRT buffer. The OS can also derive the WaveRT buffer position of the next packet to
	write within the WaveRT buffer. For WaveRT drivers, the driver signals a single notification
	event as it transfers data from each packet of the WaveRT buffer
*/
#pragma code_seg()
_IRQL_requires_max_(PASSIVE_LEVEL)
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::GetPacketCount(IMiniportWaveRTOutputStream interface))(PacketCount = %d)]",*pPacketCount));
    ASSERT(pPacketCount);
...
{
	DPF_ENTER(("CMiniportWaveRTStream::SetState"));
    PAGED_CODE();
    ...
    {
        case KSSTATE_STOP:
			DPF_ENTER(("CMiniportWaveRTStream::SetState::KSSTATE_STOP"));
            // Reset DMA
            m_llPacketCounter = 0;
            m_ullPlayPosition = 0;
            m_ullWritePosition = 0;
        ...
        case KSSTATE_ACQUIRE:
			DPF_ENTER(("CMiniportWaveRTStream::SetState::KSSTATE_ACQUIRE"));
            break;

        case KSSTATE_PAUSE:
			DPF_ENTER(("CMiniportWaveRTStream::SetState::KSSTATE_PAUSE"));
        ...
        case KSSTATE_RUN:
			DPF_ENTER(("CMiniportWaveRTStream::SetState::KSSTATE_RUN"));
            // Start DMA
...
{
	DPF_ENTER(("[CMiniportWaveRTStream::SetFormat(IMiniportWaveRTStream interface)]"));
...
--*/
{
	DPF_ENTER(("[CMiniportWaveRTStream::WriteBytes]"));
...
--*/
{
	DPF_ENTER(("[CMiniportWaveRTStream::ReadBytes]"));
    ULONG bufferOffset = m_ullLinearPosition % m_ulDmaBufferSize;
...
// A2DP write data here....
			// do not save data into disk.
